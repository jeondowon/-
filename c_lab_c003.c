...
int findRoom(int persons[5]){ 
	...
	// 후보 방들 중 랜덤 선택
	int randnum = rand() % count;   // 가능한 방 갯수만큼의 경우의 수 안에서 랜덤숫자 만들기
	int roomnum = joinable[randnum];  // '랜덤숫자'번째 방을 입실할 방으로 선택
	...
	for(room = 101; room <= 105; room++){
		...
		for(int i = 0; i < mc; i++){    // 등록된 남학생 수만큼 반복
			if(mr[i] == room){  // i번째 학생의 배정 호실이 현재 검사중인 room과 같은지 확인
				printf("%s ", mn[i]);	//i번째 방호수가 현재 검사중인 room과 같다면 i번째 학생의 이름 출력
			}
			else{
				printf("");	// 아무도 없다면 공백 출력
			}
		}
		printf("\n");
	}
